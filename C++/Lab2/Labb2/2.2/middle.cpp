...
namespace lab2{
		...
		std::vector<std::string> weeknames = {"monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"};
	...
	/*
	 int middle::mod_julian_day() const{
	 	
	 		time_t mytime;
    		k_time(&mytime);
    		
    		int sparaAr = ar;
    		int sparaManad = manad;
    		int sparaDag = dag;
    		
    		
   			struct tm *currentTime = gmtime(&mytime);
			int slutAr = (*currentTime).tm_year + 1900;
			int slutManad = (*currentTime).tm_mon + 1;
			int slutDag = (*currentTime).tm_mday;
			int modJulDay = 0;
			//Ovan är dagens datum i gregorianska kalendern
	 		...
	 		while(slutAr != startAr) {
	 		int daysPerYear = 365;
	 		
	 		if ((year() % 400 == 0) || ((year() % 4 == 0) && (year() % 100 != 0))) {
				daysPerYear = daysPerYear + 1;
			} 
	 		
	 		ar = ar + 1;
	 		startAr = startAr + 1;
	 		modJulDay = modJulDay + daysPerYear;
	 		}
	 		
	 		if(leap_year() && month() > 2) {
	 			modJulDay = modJulDay + 1;
	 		}
			
	 		
	 		//Nu är vi på samma år, och måste lägga till månader/dagar
	 		if(month() > slutManad) { 
	 			//räkna ner
	 			
	 			modJulDay = modJulDay - day() + 1;
	 			dag = 1;
	 			while(month() != slutManad) {
	 				manad = manad - 1;
	 				int dtm = days_this_month();
	 				modJulDay = modJulDay - dtm; //Kan vara fel
	 			}
	 			//Nu är vi i rätt månad, rätt år
	 			
	 	
	 		
			} else if(month() < slutManad) {
					//räkna upp
					modJulDay = modJulDay - day() + 1;
					dag = 1;
					while(month()!= slutManad) {
						manad = manad + 1;
						int dtm = days_this_month();
						modJulDay = modJulDay + dtm - 1;
						}
					}
			
			
		
	 		modJulDay = modJulDay + ( slutDag - dag);
