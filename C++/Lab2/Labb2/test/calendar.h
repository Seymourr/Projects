...
class Calendar {
	...
	template<typename S>
	Calendar<S>& operator=(const Calendar<S> & otherCal);
...
template<typename S>
	Calendar<T>::Calendar(const Calendar<S> & otherCal) {
		...
		for(int i = 0; i < otherCal.cal.size(); ++i){
			cal[i].date = otherCal.cal[i].date;
			cal[i].event = otherCal.cal[i].event;
		}
...
template<typename T>
template<typename S>
	Calendar<S>& Calendar<T>::operator=(const Calendar<S> & otherCal) {
		cal.erase(cal.begin(), cal.end());
		for(int i = 0; i < otherCal.cal.size(); ++i){
			cal[i].date = otherCal.cal[i].date;
			cal[i].event = otherCal.cal[i].event;
...
template<typename T>
		...
		if(someDate.currentTime < someDate.cal[i].date) {
				os << someDate.cal[i].date << " : " << someDate.cal[i].event << "\n";
				}
		}
		std::cout << "";
		}
